#ifndef SAY_TINYSTL_DEQUE_H
#define SAY_TINYSTL_DEQUE_H

// deque: 双端队列, 作为 queue 的适配器
...
// map 作为主控来作为掌控中心 掌控“连续空间”
// map 是一小块连续空间，其中每个元素都是指针，指向另一段 (较大的) 连续线性空间，称为缓冲区
// 缓冲区的大小由 buffer_size 决定 buffer_size 会调用该 deque_buf_size

template <class T>
struct deque_buf_size {
	static constexpr size_t value = sizeof(T) < 256 ? 4096 / sizeof(T) : 16; // 缓冲区最少分配 16个 size_t
};
...
struct deque_iterator : public iterator<random_access_iterator_tag, T>{
	...
	typedef deque_iterator<T, const T&,const T*>       iterator;
	...
	// 以下各个重载运算子是 deque_iterator<> 成功运作的关键

	reference operator*() const { return *cur; }
	Pointer operator->() const { return cur; }

	// 迭代器相减结果 = n个buffer * buffer_size + (当前 - 最前) - (xcur - xfirst)
	...
	self& operator++() {
		++cur;
		if (cur == last) { // 达到缓冲区的尾，跳到下一个缓冲区的头
			set_node(node + 1);
			cur = first;
		}
		return *this;
...
public:
	...
	template <class Iter, typename std::enable_if<
		saystl::is_input_iterator<IIter>::value, int>::type = 0>
	deque(Iter first, Iter last) {
		copy_init(first, last, iterator_category(first));
	}

	deque(std::initializer_list<value_type> ilist) {
		copy_init(ilist.begin(), ilist.end(), saystl::forward_iterator_tag());
	}

	deque(const deque& rhs) {
		copy_init(rhs.begin(), rhs.end(), saystl::forward_iterator_tag());
	}

	deque(deque&& rhs) noexcept
		:begin_(saystl::move(rhs.begin_)),
		end_(saystl::move(rhs.end_)),
		map_(rhs.map_),
...
// assign
	...
	void     assign(std::initializer_list<value_type> ilist) {
		copy_assign(ilist.begin(), ilist.end(), saystl::forward_iterator_tag{});
	}
	...
	void     push_front(value_type&& value) { emplace_front(saystl::move(value)); }
	void     push_back(value_type&& value) { emplace_back(saystl::move(value)); }
...
deque<T>& deque<T>::operator=(const deque& rhs) {
	if (this != &rhs) {
		const auto len = size();
		if (len >= rhs.size()) {
			erase(saystl::copy(rhs.begin_, rhs.end_, begin_), end_);
		}
		else {
			iterator mid = rhs.begin() + static_cast<difference_type>(len);
			saystl::copy(rhs.begin_, mid, begin_);
...
void deque<T>::emplace_front(Args&& ...args)
{
	...
	{
		data_allocator::construct(begin_.cur - 1, saystl::forward<Args>(args)...);
		--begin_.cur;
	}
	else
	{
		...
		{
			--begin_;
			data_allocator::construct(begin_.cur, saystl::forward<Args>(args)...);
		}
		catch (...)
		{
			++begin_;
...
void deque<T>::emplace_back(Args&& ...args)
{
	...
	{
		data_allocator::construct(end_.cur, saystl::forward<Args>(args)...);
		++end_.cur;
	}
	else
	{
...
typename deque<T>::iterator deque<T>::emplace(iterator pos, Args&& ...args)
{
	...
	{
		emplace_front(saystl::forward<Args>(args)...);
		return begin_;
	}
	else if (pos.cur == end_.cur)
	{
		emplace_back(saystl::forward<Args>(args)...);
		return end_ - 1;
	}
	return insert_aux(pos, saystl::forward<Args>(args)...);
}
...
deque<T>::insert(iterator position, value_type&& value)
{
	if (position.cur == begin_.cur)
	{
		emplace_front(saystl::move(value));
		return begin_;
	}
	else if (position.cur == end_.cur)
	{
	...
	{
		return insert_aux(position, saystl::move(value));
	}
}
...
void deque<T>::insert(iterator position, size_type n, const value_type& value)
{
	if (position.cur == begin_.cur)
	{
		...
		saystl::uninitialized_fill_n(new_begin, n, value);
		begin_ = new_begin;
	}
	else if (position.cur == end_.cur)
	{
		...
		auto new_end = end_ + n;
		saystl::uninitialized_fill_n(end_, n, value);
		end_ = new_end;
	}
	else
	{
...
deque<T>::erase(iterator position)
{
	...
	{
		saystl::copy_backward(begin_, position, next);
		pop_front();
	}
	else
	{
		saystl::copy(next, end_, position);
		pop_back();
	}
...
deque<T>::erase(iterator first, iterator last)
{
	...
	else
	{
		...
		{
			saystl::copy_backward(begin_, first, last);
...
void deque<T>::clear()
{
	...
	{ // 有两个以上的缓冲区
		saystl::destroy(begin_.cur, begin_.last);
		saystl::destroy(end_.first, end_.cur);
	}
	else
	{
		saystl::destroy(begin_.cur, end_.cur);
	}
	shrink_to_fit();
	end_ = begin_;
}
...
void deque<T>::swap(deque& rhs) noexcept
{
	if (this != &rhs)
	{
		...
		saystl::swap(map_, rhs.map_);
		saystl::swap(map_size_, rhs.map_size_);
	}
}
...
map_init(size_type nElem)
{
	...
	// 一个 map 要管理多个节点，最少 8 个，最多是 “所需节点数加 2”
	map_size_ = saystl::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
	try
	{
...
fill_init(size_type n, const value_type& value)
{
	...
	if (n != 0)
	{
		...
		{
			saystl::uninitialized_fill(*cur, *cur + buffer_size, value);
		}
		saystl::uninitialized_fill(end_.first, end_.cur, value);
	}
}
...
{
	const size_type n = saystl::distance(first, last);
	map_init(n);
...
copy_init(FIter first, FIter last, forward_iterator_tag)
{
	...
	{
		auto next = first;
		saystl::advance(next, buffer_size);
		saystl::uninitialized_copy(first, next, *cur);
		first = next;
	}
	saystl::uninitialized_copy(first, last, end_.first);
}
...
copy_assign(FIter first, FIter last, forward_iterator_tag)
{
	...
	const size_type len2 = saystl::distance(first, last);
	if (len1 < len2)
	{
		auto next = first;
		saystl::advance(next, len1);
		saystl::copy(first, next, begin_);
		insert_dispatch(end_, next, last, forward_iterator_tag{});
	}
	else
	{
		erase(saystl::copy(first, last, begin_), end_);
	}
}
...
{
	const size_type elems_before = position - begin_;
	value_type value_copy = value_type(saystl::forward<Args>(args)...);
	if (elems_before < (size() / 2))
	{ // 在前半段插入
		emplace_front(front());
		auto front1 = begin_;
	...
	{ // 在后半段插入
		...
		--back2;
		position = begin_ + elems_before;
		saystl::copy_backward(position, back2, back1);
	}
	*position = saystl::move(value_copy);
	return position;
}
...
fill_insert(iterator position, size_type n, const value_type& value)
{
	...
	if (elems_before < (len / 2))
	{
		...
		try
		{
			if (elems_before >= n)
			{
				...
				saystl::uninitialized_copy(begin_, begin_n, new_begin);
				begin_ = new_begin;
				saystl::copy(begin_n, position, old_begin);
				saystl::fill(position - n, position, value_copy);
			}
			else
			{
				saystl::uninitialized_fill(
				saystl::uninitialized_copy(begin_, position, new_begin), begin_, value_copy);
				begin_ = new_begin;
				saystl::fill(old_begin, position, value_copy);
			}
		}
		catch (...)
		{
	...
	else
	{
		...
		try
		{
			...
			{
				auto end_n = end_ - n;
				saystl::uninitialized_copy(end_n, end_, end_);
				end_ = new_end;
				saystl::copy_backward(position, end_n, old_end);
				saystl::fill(position, position + n, value_copy);
			}
			else
			{
				saystl::uninitialized_fill(end_, position + n, value_copy);
				saystl::uninitialized_copy(position, end_, position + n);
				end_ = new_end;
				saystl::fill(position, old_end, value_copy);
			}
		}
		catch (...)
		{
	...
	if (elems_before < (len / 2))
	{
		...
		try
		{
			if (elems_before >= n)
			{
				...
				begin_ = new_begin;
				saystl::copy(begin_n, position, old_begin);
				saystl::copy(first, last, position - n);
			}
			else
			{
				auto mid = first;
				saystl::advance(mid, n - elems_before);
				saystl::uninitialized_copy(first, mid,
					saystl::uninitialized_copy(begin_, position, new_begin));
				begin_ = new_begin;
				saystl::copy(mid, last, old_begin);
			}
		}
		catch (...)
		{
	...
	else
	{
		...
		try
		{
			if (elems_after > n)
			{
				...
				end_ = new_end;
				saystl::copy_backward(position, end_n, old_end);
				saystl::copy(first, last, position);
			}
			else
			{
				auto mid = first;
				saystl::advance(mid, elems_after);
				saystl::uninitialized_copy(position, end_,
					saystl::uninitialized_copy(mid, last, end_));
				end_ = new_end;
				saystl::copy(first, mid, position);
			}
		}
		catch (...)
		{
...
insert_dispatch(iterator position, FIter first, FIter last, forward_iterator_tag)
{
	...
	if (position.cur == begin_.cur)
	{
		...
		try
		{
			saystl::uninitialized_copy(first, last, new_begin);
			begin_ = new_begin;
		}
		catch (...)
		{
	...
	else if (position.cur == end_.cur)
	{
		...
		try
		{
			saystl::uninitialized_copy(first, last, end_);
			end_ = new_end;
		}
		catch (...)
		{
...
{
	const size_type new_map_size = saystl::max(map_size_ << 1,
...
bool operator<(const deque<T>& lhs, const deque<T>& rhs)
{
	return saystl::lexicographical_compare(
