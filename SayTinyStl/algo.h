...
namespace saystl {
	...
	// find
	// 在[first, last)区间内找到等于 value 的元素，返回指向该元素的迭代器, 找不到则返回 end()
	template <class InputIter, class T>
	InputIter
	...
	ForwardIter1
		...
		if (d1 < d2)                       // 如果 序列2 大于 序列1, 那么直接返回， 序列2 不可能 属于 序列1
			return last1;
		...
		while (current2 != last2) {
			...
			else {                         // 如果不等，相等则返回当前 last1
				if (d1 == d2) {            // 如果两个子序列一样长
					return last1;          // 表示不可能成功了     
				}
				else {                     // 否则把 first2 调回最开始，继续重新查找
					current1 = ++first1;
					current2 = first2;
					--d1;                  // 已经排除了 序列1 的一个元素，所以 序列1 的长度要减一
				}
			}
		}
		return first1;
	...
	ForwardIter
		...
		if (n <= 0)
			...
			first = saystl::find(first, last, value);         // 首先找出 value 第一次出现点
			while (first != last) {
				auto m = n - 1;                               // value 还应该出现 m 次
				auto i = first;
				++i;
				...
				if (m == 0)                                   // m==0 代表找到了，返回指向这个子序列的起始处
					return first;
				else first = saystl::find(i, last, value);    // 没有找到，找下一个 value 的出现点，并准备回到外循环
			}
			return last;
		}
	}
	...
	ForwardIter1
		...
		if (first2 == last2) {       // 如果查找目标是空的
			...
			while (true) {
				...
				if (new_result == last1) {         // 如果查找的目标是空的
					return result;                 // 返回 last1，表示该 “空子序列” 的最后出现点
				}
				else {
					result = new_result;
					first1 = new_result;
					++first1;                      // 调整标兵 准备下一次查找行动
				}
			}
		}
	}
	...
	{
		// 由于查找的是 "最后出现低点" 因此反向查找比较快  利用 reverse_iterator
	...
	// 找 序列2 任何其中一个元素第一次出现于第一序列的低点，若未找到返回 last1
	...
	// lbound_dispatch 的 forward_iterator_tag 版本 ( 应用于有序空间 )
	...
	bool is_heap(RandomIter first, RandomIter last) {
		...
		for (auto child = 1; child < n; ++child) {
			if (first[parent] < first[child])
				return false;
			// &是二进制位参与运算，末尾开始，奇数二进制末尾是1，偶数是0，也就是说奇数i&1结果为1，偶数则为0
			if ((child & 1) == 0) // & 与运算(也就是说奇数i&1结果为1，偶数则为0)  child & 1 如果等于 0 ，即 child == 偶数
				++parent;
		}
		return true;
	}
	...
	ForwardIter remove(ForwardIter first, ForwardIter last, const T& value) {
		first = saystl::find(first, last, value);  // 利用 find 找出第一个匹配的地方
		auto next = first;
	...
	ForwardIter
		...
		first = saystl::find_if(first, last, unary_pred);  // 利用 find_if 找出第一个匹配的地方
		auto next = first;
	...
	bool next_permutation(BidirectionalIter first, BidirectionalIter last) {
		...
		for (;;) {
			...
			if (*--i < *ii) {                 // 找到第一对小于关系的元素
				auto j = last;
				while (!(*i < *--j)) {}
				saystl::iter_swap(i, j);       // 交换 i，j 所指元素
				saystl::reverse(ii, last);     // 将 ii 之后的所有元素反转
				return true;
			}
			if (i == first) {
	...
	// 其余放到 result_false 的输出区间，并返回一个 saystl::pair 指向这两个区间的尾部
	...
	void intro_sort(RandomIter first, RandomIter last, Size depth_limit)
	{
		while (static_cast<size_t>(last - first) > kSmallSectionSize)
		{
			if (depth_limit == 0)
			{                      // 到达最大分割深度限制
				saystl::partial_sort(first, last, last);  // 改用 heap_sort
				return;
			}
			--depth_limit;
		...
		Size depth_limit, Compared comp)
	{
		while (static_cast<size_t>(last - first) > kSmallSectionSize)
		{
			if (depth_limit == 0)
			{                            // 到达最大分割深度限制
				saystl::partial_sort(first, last, last, comp);  // 改用 heap_sort
				return;
			}
			--depth_limit;
			auto mid = saystl::median(*(first), *(first + (last - first) / 2), *(last - 1));
