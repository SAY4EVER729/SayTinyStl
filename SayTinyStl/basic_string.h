#ifndef SAYTINYSTL_BASIC_STRING_H
#define SAYTINYSTL_BASIC_STRING_H
...
namespace saystl {
	...
	struct char_traits {
		...
		static char_type* copy(char_type* dst, const char_type* src, size_t n) {
			SAYSTL_DEBUG(src + n <= dst || dst + n <= src);
			char_type* r = dst;
			for (; n != 0; --n, ++dst, ++src) {
				*dst = *src;
			}
...
// 模板类 basic_string
// 参数一代表字符类型，参数二代表萃取字符类型的方式，缺省使用 saystl::char_traits
template <class CharType, class CharTraits = saystl::char_traits<CharType>>
class basic_string {
public:
	typedef CharTraits                                   traits_type;
	typedef CharTraits                                   char_traits;

	typedef saystl::allocator<CharType>                  allocator_type;
	typedef saystl::allocator<CharType>                  data_allocator;
...
// insert
	...
	{
		count = saystl::min(count, size_ - index);
	...
	{
		SAYSTL_DEBUG(begin() <= first && last <= end() && first <= last);
...
private:
	...
	template <class Iter>
	void          copy_init(Iter first, Iter last, saystl::input_iterator_tag);
	template <class Iter>
	void          copy_init(Iter first, Iter last, saystl::forward_iterator_tag);
...
insert(const_iterator pos, Iter first, Iter last)
{
	...
	if (pos == end())
	{
		saystl::uninitialized_copy(first, last, end());
		size_ += len;
		return r;
	}
	char_traits::move(r + len, r, len);
	saystl::uninitialized_copy(first, last, r);
	size_ += len;
	return r;
}
...
{
	auto n1 = saystl::min(count1, size_ - pos1);
...
{
	auto n1 = saystl::min(count1, size_ - pos1);
	auto n2 = saystl::min(count2, other.size_ - pos2);
...
swap(basic_string& rhs) noexcept
{
	if (this != &rhs)
	{
		saystl::swap(buffer_, rhs.buffer_);
		saystl::swap(size_, rhs.size_);
...
{
	const auto init_size = saystl::max(static_cast<size_type>(STRING_INIT_SIZE), n + 1);
...
template <class Iter>
void basic_string<CharType, CharTraits>::
copy_init(Iter first, Iter last, saystl::input_iterator_tag)
{
	size_type n = saystl::distance(first, last);
	const auto init_size = saystl::max(static_cast<size_type>(STRING_INIT_SIZE), n + 1);
	try
	{
		buffer_ = data_allocator::allocate(init_size);
...
template <class Iter>
void basic_string<CharType, CharTraits>::
copy_init(Iter first, Iter last, saystl::forward_iterator_tag)
{
	...
	try
	{
		...
		size_ = n;
		cap_ = init_size;
		saystl::uninitialized_copy(first, last, buffer_);
	}
	catch (...)
	{
		buffer_ = nullptr;
...
{
	const auto init_size = saystl::max(static_cast<size_type>(STRING_INIT_SIZE), count + 1);
	buffer_ = data_allocator::allocate(init_size);
...
append_range(Iter first, Iter last)
{
	...
	saystl::uninitialized_copy_n(first, n, buffer_ + size_);
	size_ += n;
	return *this;
}
...
{
	const auto new_cap = saystl::max(cap_ + need, cap_ + (cap_ >> 1));
...
reallocate_and_fill(iterator pos, size_type n, value_type ch)
{
	...
	const auto new_cap = saystl::max(old_cap + n, old_cap + (old_cap >> 1));
	auto new_buffer = data_allocator::allocate(new_cap);
...
reallocate_and_copy(iterator pos, const_iterator first, const_iterator last)
{
	...
	auto new_buffer = data_allocator::allocate(new_cap);
	auto e1 = char_traits::move(new_buffer, buffer_, r) + r;
	auto e2 = saystl::uninitialized_copy_n(first, n, e1) + n;
	char_traits::move(e2, buffer_ + r, size_ - r);
	data_allocator::deallocate(buffer_, old_cap);
	buffer_ = new_buffer;
	size_ += n;
...
operator+(basic_string<CharType, CharTraits>&& lhs,
	const basic_string<CharType, CharTraits>& rhs)
{
	basic_string<CharType, CharTraits> tmp(saystl::move(lhs));
	tmp.append(rhs);
	return tmp;
}
...
{
	basic_string<CharType, CharTraits> tmp(saystl::move(rhs));
...
#endif // !SAYTINYSTL_BASIC_STRING_H
